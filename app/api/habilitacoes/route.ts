...
export async function POST(req: NextRequest) {
  ...
  // Vincular: duplicate an existing doc into a new licitação
  if (body.vincular === true) {
    const { docId, licitacaoGoverno } = body;
    if (!docId || !licitacaoGoverno) {
      return NextResponse.json({ error: 'docId e licitacaoGoverno obrigatórios' }, { status: 400 });
    }

    const source = await sql`
      SELECT * FROM licitacoes_habilitacoes
      WHERE licitacoes_habilitacao_id = ${docId}
    ...
    const result = await sql`
      ...
      VALUES (
        ${doc.licitacoes_habilitacao_nome}, ${doc.licitacoes_habilitacao_documento || ''},
        ${hash}, ${userId},
        ${doc.licitacoes_habilitacao_data_validade || null}, ${userId},
        ${licitacaoGoverno}
      )
      RETURNING *
    `;
